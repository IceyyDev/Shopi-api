        ...
        try:
            ...
            if payment_data.get("__typename") == "FilledPaymentTerms":
                ...
                for line in avail_lines:
                    ...
                    # ACCEPT only if it matches Shopify Payments patterns
                    is_shopify_payments = (
                        any(sp in lower_pid for sp in SHOPIFY_PAYMENTS_IDENTIFIERS) or
                        any(sp in lower_gateway for sp in SHOPIFY_PAYMENTS_NAMES) or
                        pm_type == "PaymentProvider" and "shopify" in lower_gateway.lower()
                    )
                    
                    # Also accept if identifier looks like Shopify's format (alphanumeric hash)
                    # Shopify Payments typically has identifiers like "c_12345..." or just card identifiers
                    if not is_shopify_payments:
                        # Check if it's a simple credit card type gateway (not a third-party processor)
                        if pm_type == "PaymentProvider" and not any(rg in lower_gateway for rg in REJECTED_GATEWAYS):
                            # Only accept if the name suggests direct card payment
                            if any(term in lower_gateway for term in ["card", "credit", "debit", "direct"]):
                                is_shopify_payments = True
                    
                    if is_shopify_payments:
                        payment_identifier = pid
                        gateway = detected_gateway if detected_gateway else "Shopify Payments"
                        ...
                        try:
                            ...
                            pass
                        log.debug("Selected Shopify Payments gateway: %s (pid=%s)", gateway, pid)
                        break
            ...
            try:
                ...
                if "errors" in d_resp and "data" not in d_resp:
                    ...
                    d_gql_errs = d_resp.get("errors", [])
                    if d_gql_errs:
                        d_code = _parse_gql_errors(d_gql_errs)
                        if d_code in ("SESSION_EXPIRED", "SITE_REQUIRES_LOGIN",
                                      "THROTTLED", "NO_PRODUCT"):
                            return _r(d_code)
                ...
                if _del_sst:
                    sst = _del_sst
                    gql_headers["x-checkout-one-session-token"] = sst
                # Check negotiate-level errors from delivery step
                d_negotiate = (
                    d_resp.get("data", {}).get("session", {})
                    .get("negotiate", {})
                )
                d_neg_errors = (d_negotiate or {}).get("errors") or []
                if d_neg_errors:
                    d_neg_code = _parse_gql_errors(d_neg_errors)
                    if d_neg_code not in ("GRAPHQL_ERROR", "CARD_DECLINED"):
                        return _r(d_neg_code)

                # Handle SubmittedForCompletion from delivery step (digital goods / auto-submit)
                d_result = (d_negotiate or {}).get("result", {})
                if d_result:
